...
% threshold parameter
m2=0.05;
max_delta2=2.5;%max_phi=max_delta*m1
min_delta2=1.0;%min_phi=min_delta*m1
max_delta_r=0.12;
min_delta_r=0.08;
topm2=0.145;%topm1>=max_phi/(1-max_delta_r)
vare=1/5;Count_2=0;r2=1;

while n<=(tm/h)+1;
    if n==1
     %% Initialization
        x2(1,:)=[-4,3.5,3.0];
        D_es12(1)=0.0;D_es22(1)=0.0;D_es32(1)=0.0;
        d1_es2(1)=0;
        rho_es2(1,:)=[0.0 0.0 0.0];
        vd2(1,:)=[0.0 0.0 0.0];
        u2(1)=0;
        d12(1)=0;intervals2(1)=1;
        z2(1,:)=[0.35 0.35 0.35];
        w2(1)=0;delta2(1)=1.0;
     ...
     %%  ADOB 
        ...
        D_es12(n)=h*(-0.00*D_es12(n-1)+1*B'*P*x2(n-1,:)'*rho_es2(n-1,1))+D_es12(n-1);
        D_es22(n)=h*(-0.00*D_es12(n-1)+1*B'*P*x2(n-1,:)'*rho_es2(n-1,2))+D_es22(n-1);
        D_es32(n)=h*(-0.00*D_es12(n-1)+1*B'*P*x2(n-1,:)'*rho_es2(n-1,3))+D_es32(n-1);
     ...
     %% ET     
        e=w2(n)-u2(n-1);
        delta2(n)=max_delta2-(max_delta2-min_delta2)*tanh((10*e*e)/(u2(n-1)*u2(n-1)));
        if delta2(n)>=max_delta2
            delta2(n)=max_delta2;
        end
        if delta2(n)<=min_delta2
            delta2(n)=min_delta2;
        end
        ...
        if norm(e,inf)>(delta_r(n)*norm(u2(n-1),inf)+delta2(n)*m2)
        u2(n)=w2(n);
        Count_2=Count_2+1;
        Lount_1(n)=1;
        intervals2(n)=(n-r2)*h;
